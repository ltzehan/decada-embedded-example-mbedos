...
/**
 * SecureElementSocketWrapper is a wrapper around Socket for interacting with TLS servers.
 *
 * SecureElementSocketWrapper is identical to TLSSocketWrapper except for the lifecycle management
 * of the mbedtls_pk_context _pkctx, instead using a reference to a mbedtls_pk_context pre-configured 
 * with the client's private key. This removes the need to pass in the private key in plaintext, 
 * deferring to the SecureElement to perform cryptographic operations on the opaque private key.
 *
 */
class SecureElementSocketWrapper : public Socket {
public:
    /** Transport modes */
    ...
    /** Create a SecureElementSocketWrapper.
     *
     ...
     */
    SecureElementSocketWrapper(Socket *transport, const char *hostname = NULL, control_transport control = TRANSPORT_CONNECT_AND_CLOSE);

    /** Destroy a socket wrapper.
     *
     *  Closes socket wrapper if the socket wrapper is still open.
     */
    ~SecureElementSocketWrapper() override;
    ...
    /** (Modified from TLSSocket) Sets client certificate.
     *
     ...
     * @retval NSAPI_ERROR_PARAMETER in case the provided root_ca parameter failed parsing.
     */
    nsapi_error_t set_client_cert(const void *client_cert, size_t client_cert_len);

    /** (Modified from TLSSocket) Sets client certificate, and mbedtls_pk_context for client private key.
     *
     * @param client_cert_pem Client certification in PEM format.
     * @param mbedtls_pk_ctx mbedtls_pk_context configured with client private key.
     * @retval NSAPI_ERROR_OK on success.
     * @retval NSAPI_ERROR_PARAMETER in case the provided root_ca parameter failed parsing.
     */
    nsapi_error_t set_client_cert_key(const char *client_cert_pem, const mbedtls_pk_context& mbedtls_pk_ctx);
...
#endif // SECURE_ELEMENT_SOCKET_WRAPPER_H
