...
/* (Modified from TLSSocket) _pkctx is no longer initialized in the constructor */
SecureElementSocketWrapper::SecureElementSocketWrapper(Socket *transport, const char *hostname, control_transport control) :
...
/* (Modified from TLSSocket) _pkctx is no longer freed in the destructor */
SecureElementSocketWrapper::~SecureElementSocketWrapper()
{
    if (_transport) {
        close();
    }
...
void SecureElementSocketWrapper::set_hostname(const char *hostname)
{
...
nsapi_error_t SecureElementSocketWrapper::set_root_ca_cert(const void *root_ca, size_t len)
{
#if !defined(MBEDTLS_X509_CRT_PARSE_C)
...
nsapi_error_t SecureElementSocketWrapper::set_root_ca_cert(const char *root_ca_pem)
{
    return set_root_ca_cert(root_ca_pem, strlen(root_ca_pem) + 1);
}

/* (Modified from TLSSocket) Internal mbedtls_pk_context is assigned to pre-configured context */
nsapi_error_t SecureElementSocketWrapper::set_client_cert_key(const char *client_cert_pem, const mbedtls_pk_context& mbedtls_pk_ctx)
{
    _pkctx = mbedtls_pk_ctx;
    return set_client_cert(client_cert_pem, strlen(client_cert_pem) + 1);
}

/* (Modified from TLSSocket) Private key is no longer parsed from plaintext */
nsapi_error_t SecureElementSocketWrapper::set_client_cert(const void *client_cert, size_t client_cert_len)
{
...
nsapi_error_t SecureElementSocketWrapper::start_handshake(bool first_call)
{
...
#else
#error "CTR or HMAC must be defined for SecureElementSocketWrapper!"
#endif
    ...
    _transport->sigio(mbed::callback(this, &SecureElementSocketWrapper::event));
...
nsapi_error_t SecureElementSocketWrapper::continue_handshake()
{
    int ret;
...
nsapi_error_t SecureElementSocketWrapper::send(const void *data, nsapi_size_t size)
{
    int ret;
...
nsapi_size_or_error_t SecureElementSocketWrapper::sendto(const SocketAddress &, const void *data, nsapi_size_t size)
{
    // Ignore the SocketAddress
    return send(data, size);
}

nsapi_size_or_error_t SecureElementSocketWrapper::recv(void *data, nsapi_size_t size)
{
    int ret;
...
nsapi_size_or_error_t SecureElementSocketWrapper::recvfrom(SocketAddress *address, void *data, nsapi_size_t size)
{
    if (address) {
        getpeername(address);
    }
...
void SecureElementSocketWrapper::print_mbedtls_error(MBED_UNUSED const char *name, MBED_UNUSED int err)
{
...
void SecureElementSocketWrapper::my_debug(void *ctx, int level, const char *file, int line,
                                const char *str)
{
...
int SecureElementSocketWrapper::my_verify(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
...
int SecureElementSocketWrapper::ssl_recv(void *ctx, unsigned char *buf, size_t len)
{
    int recv;

    SecureElementSocketWrapper *my = static_cast<SecureElementSocketWrapper *>(ctx);
...
int SecureElementSocketWrapper::ssl_send(void *ctx, const unsigned char *buf, size_t len)
{
    int size = -1;
    SecureElementSocketWrapper *my = static_cast<SecureElementSocketWrapper *>(ctx);
...
mbedtls_x509_crt *SecureElementSocketWrapper::get_own_cert()
{
    return _clicert;
}

int SecureElementSocketWrapper::set_own_cert(mbedtls_x509_crt *crt)
{
    int ret = 0;
...
mbedtls_x509_crt *SecureElementSocketWrapper::get_ca_chain()
{
    return _cacert;
}

void SecureElementSocketWrapper::set_ca_chain(mbedtls_x509_crt *crt)
{
...
mbedtls_ssl_config *SecureElementSocketWrapper::get_ssl_config()
{
    if (!_ssl_conf) {
        int ret;
...
void SecureElementSocketWrapper::set_ssl_config(mbedtls_ssl_config *conf)
{
...
mbedtls_ssl_context *SecureElementSocketWrapper::get_ssl_context()
{
    return &_ssl;
}

nsapi_error_t SecureElementSocketWrapper::close()
{
    if (!_transport) {
        return NSAPI_ERROR_NO_SOCKET;
    }
...
nsapi_error_t SecureElementSocketWrapper::connect(const SocketAddress &address)
{
...
nsapi_error_t SecureElementSocketWrapper::bind(const SocketAddress &address)
{
    if (!_transport) {
        return NSAPI_ERROR_NO_SOCKET;
    }
...
void SecureElementSocketWrapper::set_blocking(bool blocking)
{
    set_timeout(blocking ? -1 : 0);
}

void SecureElementSocketWrapper::set_timeout(int timeout)
{
    _timeout = timeout;
...
void SecureElementSocketWrapper::sigio(mbed::Callback<void()> func)
{
    if (!_transport) {
        return;
    }
...
nsapi_error_t SecureElementSocketWrapper::setsockopt(int level, int optname, const void *optval, unsigned optlen)
{
    if (!_transport) {
        return NSAPI_ERROR_NO_SOCKET;
    }
...
nsapi_error_t SecureElementSocketWrapper::getsockopt(int level, int optname, void *optval, unsigned *optlen)
{
    if (!_transport) {
        return NSAPI_ERROR_NO_SOCKET;
    }
...
Socket *SecureElementSocketWrapper::accept(nsapi_error_t *err)
{
    if (err) {
...
nsapi_error_t SecureElementSocketWrapper::listen(int)
{
    return NSAPI_ERROR_UNSUPPORTED;
}

void SecureElementSocketWrapper::event()
{
    _event_flag.set(1);
    if (_sigio) {
        _sigio();
...
bool SecureElementSocketWrapper::is_handshake_started() const
{
    return _tls_initialized;
}


nsapi_error_t SecureElementSocketWrapper::getpeername(SocketAddress *address)
{
    if (!_handshake_completed) {
        return NSAPI_ERROR_NO_CONNECTION;
    }
